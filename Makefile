...
# Run tests once
test-once:
	npm test -- --no-watch && sleep 2 && pkill -f 'karma-chrome-launcher' || true
...
e2e-run: kill-server
	(npm start -- --port 4201 &) && sleep 10 && CYPRESS_BASE_URL=http://localhost:4201 npm run cypress:run && pkill -f "ng serve" && sleep 2 && pkill -f 'cypress' || true
